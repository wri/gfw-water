    ...
    return (
      <div className='control-panel map-component shadow'>
        <ul>
          ...
          <li className={`hide-controls pointer mobile-hide${!this.state.controlsVisible ? ' hide-controls--on' : ''}`} title='Reset' onClick={::this.toggleControls}>
            <svg className='panel-icon' dangerouslySetInnerHTML={{ __html: (this.state.controlsVisible ? toggleSvgOff : toggleSvgOn) }}/>
          </li>
          <li className='basemap-layers pointer mobile-hide' title='Basemaps' onClick={::this.toggleBasemapGallery}>
